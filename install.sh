#!/usr/bin/env bash
# Install fable-mythos-controller as an OpenCode plugin.
#
# Discovers the controller repo (this directory), copies the plugin layout
# into ~/.opencode/plugins/fable-mythos-controller, and verifies the install.
#
# Usage:
#   bash install.sh           # installs to ~/.opencode/plugins/fable-mythos-controller
...
SKILL_DIR="$HOME/.opencode/skills/$PLUGIN_NAME"
...
if [[ $UNINSTALL -eq 1 ]]; then
  ...
  run rm -rf "$SKILL_DIR"
  echo "Done. Run /plugins reload in OpenCode."
  exit 0
fi
...
# ---- Install as OpenCode plugin (manifest + skills) ----
...
echo ""
echo "1. Reload OpenCode plugins."
echo ""
