...
description: |
  ...
  the deterministic Python controller at ~/.opencode/skills/fable-mythos-controller/scripts/run-controller.sh.
...
4. **Invoke the controller** via the helper script. The script handles
   ...
   ```bash
   bash ~/.opencode/skills/fable-mythos-controller/scripts/run-controller.sh \
...
- Controller adapters that spawn real OpenCode agents require an active
  OpenCode login. Without login, the adapter writes a STUB transcript and
